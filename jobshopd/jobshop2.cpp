...
#define START auto t1 = chrono::high_resolution_clock::now();
#define STOP auto t2 = chrono::high_resolution_clock::now(); double duration = (double)chrono::duration_cast<chrono::microseconds>(t2-t1).count();
...
void saveAns(Ans* ans)
{
    ...
    {
        fstream file2; 
        file2.open("output/times.txt", ios::app);
        file2 << fileName2 << "\t" << ans->jobs << "\t" << ans->machines << "\t" << czas << "\t" << sum/10 << endl;
        file2.close();
    }
